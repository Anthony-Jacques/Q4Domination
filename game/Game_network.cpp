...
void idGameLocal::ServerWriteInitialReliableMessages( int clientNum ) {
	...
	for(i = 0; i < MAX_GENTITIES; i++ ) 
	{
		if (gameLocal.entities[i] && gameLocal.entities[i]->IsType(ajDominationPoint::GetClassType()))
			((ajDominationPoint*)gameLocal.entities[i])->WriteDominationPointChange(false);
	}
...
// RAVEN END
		case GAME_RELIABLE_MESSAGE_DOMPOINT_STATECHANGE:
		{
			ajDominationPoint::ReadDominationPointChange(msg);
			break;
		}
...
// shouchard:  multifield vote stuff
		case GAME_RELIABLE_MESSAGE_STARTPACKEDVOTE: {
			...
			if ( 0 != ( voteData.m_fieldFlags & VOTEFLAG_DOMINATIONLIMIT ) ) {
				voteData.m_dominationLimit = msg.ReadShort();
			}
...
============
*/
void idGameLocal::SetDemoState( demoState_t state,  bool serverDemo ) {
	demoState = state;
}
