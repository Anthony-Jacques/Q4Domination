...
#define	GAME_VERSION		"DominationQ4"
...
// RAVEN END	
	GAME_RELIABLE_MESSAGE_DOMPOINT_STATECHANGE,
};
...
// RAVEN END
	...
	int						GetDemoFollowClient( void ) const { return 1; } //{ return serverDemo ? followPlayer : -1; }
	idUserInterface			*GetDemoHud( void ) { return NULL; }
	idUserInterface			*GetDemoMphud( void ) { return NULL; }
	idUserInterface			*GetDemoCursor( void ) { return NULL; }
...
// RAVEN END
	...
	///// ADJ
	idHashTable<idStr>*		m_replaceMap;
	idHashTable<idStr>*		m_domNameMap;
	void					CheckReplaceList(idDict &args, bool useDefaultReplacements);
	void					BuildReplaceList(const char *mapName);
	void					ParseReplaceListEntry(char *line);
};
...
ID_INLINE bool idGameLocal::IsTeamGame ( void ) const {
	return ( isMultiplayer && ( gameType == GAME_CTF || gameType == GAME_TDM || gameType == GAME_1F_CTF || gameType == GAME_ARENA_CTF || gameType == GAME_DOMINATION) );
}
